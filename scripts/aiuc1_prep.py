...
def run_harness(url: str, harnesses: list[str] | None = None, simulate: bool = False) -> list[dict]:
    ...
    for suite in suites:
        ...
        if simulate:
            ...
            cmd.extend(["--url", url])
        cmd.extend(["--report", f"/tmp/aiuc1_{suite}_report.json"])
        ...
        try:
            ...
            report_path = f"/tmp/aiuc1_{suite}_report.json"
...
def generate_readiness_report(statuses: list[RequirementStatus], target: str = "N/A") -> str:
    ...
    lines = [
        ...
        f"**Target:** {target}",
        f"**Framework:** Agent Security Harness v3.8",
    ...
    lines.extend([
        ...
        f"---",
        f"",
        f"*Report generated by Agent Security Harness v3.8 AIUC-1 Prep Tool*",
