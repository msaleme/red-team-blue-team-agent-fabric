...
| Repository tests | 640 (`python scripts/count_tests.py`) |
...
| 1.6 | v1.1 `65e3bd59f99c` | 4.21.3 | `066d2a957d51` | 2026-09-10 | Repository test total 623 -> 627 on the addition of MEM-013..MEM-016, the memory revocation-enforcement family (arXiv:2609.08258, which reports that across five agent-memory systems no system enforces revocation by default). No threat, scenario or control verdict changed and no adjudication was redone. The four are ASI07/ASI03 tests on surfaces this mapping already adjudicates and none is enumerated in a scenario here, so unique_mapped_tests is unchanged at 96 -- this mapping does not claim them. Recorded because r15_total compares the stated total against a live count, so a total left stale would fail rather than mislead. commit and assessed_at are deliberately unchanged from 1.1. |
| 1.7 | v1.1 `65e3bd59f99c` | 4.21.3 | `72af259ca36e` | 2026-09-10 | Repository test total 627 -> 631 on the addition of WT-001..WT-004, the workspace-trust / repository-ingestion pre-flight family (GitSpawn, Manifold Security 2026-09-01; CVE-2026-72718, CVE-2026-71963). Test-bearing modules 45 -> 46. These are ASI06 tests on a surface this mapping does NOT currently adjudicate -- it maps protocol-layer and model-layer threats, and this family fires before the first protocol byte, in the same position as the MCP-F-* pre-flight family. None is enumerated in a scenario here, so unique_mapped_tests is unchanged at 96 and this mapping does not claim them. Recorded because r15_total compares the stated total against a live count, so a total left stale would fail rather than mislead. commit and assessed_at are deliberately unchanged from 1.1. |
| 1.9 | v1.1 `65e3bd59f99c` | 4.21.3 | `HEAD` | 2026-09-21 | Repository test total 635 -> 640 on the addition of CR-001..CR-005, the capability-residue family (a seeded canary capability recovered from emitted artifacts, and a grant that outlives its revocation). Test-bearing modules 47 -> 48. All five carry an ASI04 primary on their own results and are attributable, so the corpus ASI04 count moves 65 -> 70. This mapping does NOT adjudicate them; no scenario here enumerates them, so unique_mapped_tests is unchanged and this mapping claims none of them. Recorded because r15_total compares the stated total against a live count. |
| 1.8 | v1.1 `65e3bd59f99c` | 4.21.3 | `72af259ca36e` | 2026-09-11 | Repository test total 631 -> 635 on the addition of HI-001..HI-004, the hidden-instruction family (Unicode tag-block payload, U+E0000 to U+E007F, reaching a credentialed irreversible action). Test-bearing modules 46 -> 47. The tests carry ASI01 and ASI02 primaries on their own results, but this mapping does NOT adjudicate them; no scenario here enumerates them, so unique_mapped_tests is unchanged at 96 and this mapping claims none of them. Recorded because r15_total compares the stated total against a live count, so a total left stale would fail rather than mislead. commit and assessed_at are deliberately unchanged from 1.1, following the 1.6 and 1.7 precedent. |
