...
| [EVIDENCE-INTEGRITY-OPERATING-RULES.md](EVIDENCE-INTEGRITY-OPERATING-RULES.md) | **How this repository measures itself** - a register reports a derived denominator, a static detector proves it can catch a seeded violation, and a D-and-E result is classified by evidence type before it is a change candidate. Each rule names where it is enforced. |
| [RCL-CROSS-IMPLEMENTATION-INTEROP.md](RCL-CROSS-IMPLEMENTATION-INTEROP.md) | **The one external reproduction** - a separately written Node verifier over the byte-pinned RCL corpus, 11/11 verdicts matched including both acceptance controls, two contract defects surfaced. E2/I1 for the corpus; the harness stays I0. |
| [EXTERNAL-FIXTURES.md](EXTERNAL-FIXTURES.md) | **Running someone else's fixtures** - the pinned-checker runner, its four states, the positive-control rule, and the result bundle it publishes. Raw output only; no certification or endorsement. |
| [cve/README.md](cve/README.md) | **CVE to test index** - publicly disclosed CVEs the test source cites, the tests that exercise each one's class, how to run them and what each verdict means. Generated from the source and checked for drift. |
...
The harness spans 48 test-bearing modules. A representative subset (protocol
...
**Total: 640 security tests across 48 test-bearing modules** (verified 2026-09-21 via `scripts/count_tests.py`)
