...
The Agent Security Harness is an open-source adversarial testing framework (635 tests, 47 test-bearing modules) that maps directly to AIUC-1 certification requirements. We propose inclusion as a reference implementation tool in AIUC-1 implementation guidance, providing organizations with an executable path from requirement to evidence.
