...
every URL-taking harness against a closed port, a 404-everywhere and a bare
403-everywhere target; a PASS or FAIL there is a defect, and the remainder is a
register that may only shrink (578 cells in 20 harnesses at introduction, 340 in
13 now). Behaviour changes an installed user will see:

  - A2A: a bare 401/403 to everything is no surface; seven cells leave the
    register (#601).
  - cloud-agents (all 25 tests) and the enterprise, extended-enterprise and
    framework adapter families count a rejection only after the route served a
    benign baseline request, which is sent before the first attack on each route
    (#602, #605). PA-003 no longer PASSes on a host that refuses everything.
  - CrewAI CVE tests CREW-001/004/006..010 and CVE-005 count an HTTP error as a
    refusal only when its body is an answer (#603); CVE-005 is a differential over
    a tool the target lists (#604).
  - CVE-006 judges the target's `tools/list` instead of the harness's own scanner,
    so default `mcp-tool-poisoning` runs no longer always exit `1` (#599).
  - A no-surface run in these families exits `2` (INCONCLUSIVE). For cloud-agents and
    the three adapter families it exited `0`.

The v4.15.0 statement that unserviced requests are no longer recorded as passes did
not hold in these families through v4.23.0; the CVE pages that described it
(CREW-008, PA-003 and the other CrewAI pages) are re-pinned to 3c3a66b.
...
seven CVE tests, #594 for six A2A tests), and each fix covered only the tests someone
had opened. `scripts/no_surface_sweep.py` runs every live-target harness against three
targets with nothing to judge, reusing the dead-host sweep's machinery: a closed port, a
host answering HTTP 404 to every request, and a host answering a bare HTTP 403 to every
request. On all three the only honest verdict is INCONCLUSIVE (or NOT_EXECUTED).
`testing/test_verdicts_need_a_surface.py` fails on any PASS or FAIL there.

- **Population is derived.** Every harness in the CLI registry that the dead-host sweep
  runs and whose own argparse accepts `--url` (41 harnesses, 611 tests per pole after
  3 named self-tests). A harness registered later is swept without editing the guard; a
  test seeds one and checks it. The four exercised harnesses with no `--url`
  (workspace-trust, capability-residue, hidden-instruction, skill-security) are
  excluded by a declared list, each with a reason, that must equal the derived set.
...
- **Grandfather register.** `VERDICT_WITHOUT_SURFACE` held 578 cells across 20
  ...
  seeded harness stays green while still able to PASS and FAIL against a served
  surface. The register is also listed in `test_evidence_integrity_registers`.
...
- `docs/cve/`: one page per publicly disclosed CVE that `protocol_tests/` cites,
  mapping it to the tests that exercise its vulnerability class, with facts from
  NVD, the GitHub Advisory Database and the CISA KEV catalog, the command to run
  each test, and what each verdict means. Generated by
  `scripts/generate_cve_pages.py` from `docs/cve/cve-test-mapping.yaml`;
  `testing/test_cve_pages_are_current.py` fails when the source cites a CVE or
  pairs a test the mapping does not account for, when a test ID does not exist,
  or when a page drifts. Nine CVEs documented, eight cited CVEs excluded with the
  reason recorded on the index.
